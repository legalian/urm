...
ParseVariant& ParseVariant::then(ParseArtifact obj) {
    arguments.push_back(obj);
    return *this;
}
ParseVariant& ParseVariant::then(int obj) {
    arguments.push_back(ParseArtifact(obj,0,0,0));
    return *this;
}
...
//}
//
//Construction Construction::reference(std::vector<int> path) {
//    if (path.size()==0) return *this;
//    if (reconstruct!=1) throw;
//    int p=e.path[0];
//    path.erase(path.begin());
//    if (children.size()<=p) throw;
//    return children[p].reference(e);
//}
